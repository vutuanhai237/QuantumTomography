...
# ----------------------------- #
tomog = KrausTomography()

# -------------------------------------------------------- #
# 2. Initialize with 1 qubit, 3 probe states, 1 Kraus op
# -------------------------------------------------------- #
tomog.init(num_qubits=1, num_rho=3, num_kraus=1)

# -------------------------------------------------------- #
# 3. Define epsilon (unitary used for inverse transformation)
# -------------------------------------------------------- #
epsilon = generator.haar(2**1)  # 1 qubit => 2x2 unitary
...
# Attach process function to tomography object (with fixed epsilon)

tomog.data_process_fn = partial(data_process, epsilon=epsilon)
# -------------------------------------------------------- #
# 5. Set the gradient function for optimization
# -------------------------------------------------------- #
tomog.gradient_fn = gradient.calculate_gradient
...
rho_target_set = []
for rho in rho_set:
    rho_output, rho_target, fide = tomog.evaluate(rho, partial(quantum_ops.apply_unitary, unitary=epsilon))
    print(f"\nInput ρ]:\n{rho}")
